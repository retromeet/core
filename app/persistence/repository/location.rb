    ...
    module Location
      class << self
        ...
        def upsert_location(location_result:)
          ...
          account_informations.merge_using(Database.connection.select(Sequel.as(location_result.osm_id, :osm_id)).as(:u), Sequel[:locations][:osm_id] => Sequel[:u][:osm_id])
                              .merge_insert(osm_id: :osm_id, display_name: display_name_json, country_code: location_result.country_code, geom:)
          ...
          account_informations.where(osm_id: location_result.osm_id).get(:id)
        end
